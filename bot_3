...
class XTBTradingBot:
   ...
   def check_trading_signals(self, df):
    # Si aucune position n'est ouverte, on prend une position
    if not self.position_open:
        current_time = time.time()
        
        # On alterne entre BUY et SELL toutes les minutes
        if int(current_time) % 2 == 0:
            logging.info("🔵 Prise de position ACHAT")
            return "BUY"
        else:
            logging.info("🔴 Prise de position VENTE")
            return "SELL"
    ...
    def execute_trade(self, signal):
        ...
        try:
            ...
            lot_min = float(symbol_info.get('lotMin', 0.01))  # Volume minimum

            if ask_price <= 0 or bid_price <= 0:
                logging.error("❌ Prix invalides")
                return
            ...
            if signal == "BUY":
                ...
                tp_price = entry_price * 1.01  # TP à +1%
            else:  # SELL
                entry_price = bid_price
                sl_price = entry_price * 1.01  # SL à +1%
                tp_price = entry_price * 0.99  # TP à -1%
            ...
            if response.get('status'):
                ...
                logging.error(f"❌ Échec du trade: {response}")
   ...
   def check_trade_status(self):
        try:
            ...
            logging.error(f"❌ Erreur vérification trade: {str(e)}")
            return False
